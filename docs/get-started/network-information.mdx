...
For every network, we provide two endpoints: a public RPC endpoint and a relay
endpoint. The public RPC endpoint can be consumed by Etherlink users and
...
`eth_blockNumber` available. The relay endpoint should be preferred when
setting up an [Etherlink node](/network/evm-nodes).
